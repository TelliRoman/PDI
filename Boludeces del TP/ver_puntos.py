...
# Cargar imagen
imagen = cv2.imread(r"C:\Users\pablo\Desktop\PDI\PDI\Trabajo Final de PDI\quemiras.jpg")
if imagen is None:
...
with mp_holistic.Holistic(
    ...
    min_tracking_confidence=0.5        # Confianza mínima para seguir puntos
        ...
        if results.pose_landmarks:
            ...
            prediction="buena"
            label = "-" if prediction == "buena" else "-"
        ...
        if results.face_landmarks:
            ...
            if angulo > 40:
                ...
                cv2.putText(annotated, label, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,0), 4)
            else:
...
agrandada = cv2.resize(annotated, None, fx=2.0, fy=2.0, interpolation=cv2.INTER_LINEAR)
cv2.imshow("Resultado", agrandada)
cv2.imwrite("resultador.jpg", annotated)
cv2.waitKey(0)
