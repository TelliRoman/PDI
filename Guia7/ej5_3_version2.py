...
# --- USO DE HOUGHLINESP PARA SEGMENTOS ---

# Parámetros ajustables según la imagen
minLineLength = 100  # Puedes ajustar este valor
maxLineGap = 10      # Puedes ajustar este valor

segmentos1 = cv.HoughLinesP(bordes1, 1, np.pi/180, 100, minLineLength=minLineLength, maxLineGap=maxLineGap)
segmentos2 = cv.HoughLinesP(bordes2, 1, np.pi/180, 100, minLineLength=minLineLength, maxLineGap=maxLineGap)

img1_filtro = cv.cvtColor(img1_filtro, cv.COLOR_GRAY2RGB)
img2_filtro = cv.cvtColor(img2_filtro, cv.COLOR_GRAY2RGB)

# Dibuja solo el segmento más largo (la pista principal) en cada imagen
if segmentos1 is not None:
    max_len = 0
    mejor_segmento = None
    for seg in segmentos1:
        ...
        if longitud > max_len:
            max_len = longitud
            mejor_segmento = (x1, y1, x2, y2)
    if mejor_segmento is not None:
        x1, y1, x2, y2 = mejor_segmento
        cv.line(img1_filtro, (x1, y1), (x2, y2), (255, 0, 0), 4)

if segmentos2 is not None:
    max_len = 0
    mejor_segmento = None
    ...
    if mejor_segmento is not None:
        x1, y1, x2, y2 = mejor_segmento
        cv.line(img2_filtro, (x1, y1), (x2, y2), (255, 0, 0), 4)

plt.figure(1)
plt.imshow(img1_filtro)
plt.title("Pista principal resaltada (corrientes)")
plt.figure(2)
plt.imshow(img2_filtro)
plt.title("Pista principal resaltada (iguazu)")
plt.show()